...
export interface BookCfg {
  ...
  // Image-based covers (png/webp/jpg/...). Takes priority over painters when present.
  images?: {
    front?: string;
    back?: string;
    spine?: string;
...
export interface BooksShowcaseProps {
  ...
  onOpenReader?: (book: BookCfg) => void;
}
...
class CustomTimer {
  ...
  update(timestamp?: number) {
    ...
    this.delta = Math.min(now - this.previousTime, 0.1);
...
export function BooksShowcase({
  ...
  useEffect(() => {
    ...
    (function buildLeaves() {
      ...
      const cols = [0xf43f5e, 0xf472b6, 0xfb7185, 0xe11d48];
      for (let i = 0; i < 16; i++) {
    ...
    function computeSlots() {
      ...
      const fit = portrait ? clamp(a / 1.08, 0.48, 0.78) : clamp(a / 1.62, 0.58, 0.98);
      ...
      SLOTS.hero = SLOTS.portrait
        ? [
          { p: [-1.32, -0.48, -0.12], r: [-0.045, 0.38, 0.185], s: 1.35 },
          { p: [0.16, -0.25, 0.5], r: [-0.05, -0.1, -0.035], s: 1.48 },
          { p: [1.52, -0.50, -0.34], r: [-0.045, -0.4, -0.17], s: 1.35 },
        ]
        : [
          { p: [-1.88, -0.48, -0.12], r: [-0.045, 0.38, 0.185], s: 1.32 },
          { p: [0.22, -0.30, 0.5], r: [-0.05, -0.1, -0.035], s: 1.45 },
          { p: [2.12, -0.52, -0.34], r: [-0.045, -0.4, -0.17], s: 1.32 },
        ];

      if (!showDetailPanel) {
        SLOTS.detail = { p: [0, -0.05, 0.75], r: [0.02, -0.34, 0.05], s: SLOTS.portrait ? 1.15 : 1.28 };
        return;
      }
      ...
      if (SLOTS.portrait) {
        ...
        const s = clamp(availW / fit / 2.65, 0.58, 1.25);
        SLOTS.detail = { p: [0, (yw - rootY) / fit, 0.8], r: [-0.02, -0.4, 0.06], s };
      } else {
        SLOTS.detail = { p: [-1.68, 0.0, 0.85], r: [0.02, -0.44, 0.08], s: 1.45 };
      }
    }
  ...
  return (
    <div
      ...
      data-state={uiMode}
      className={cn(
        'book-showcase relative isolate h-full min-h-[560px] overflow-hidden font-sans outline-none [container-type:size] [-webkit-tap-highlight-color:transparent] rounded-3xl border border-white/10 shadow-2xl',
    ...
    >
      ...
      >
        <span className="block whitespace-nowrap text-current text-[clamp(4.5rem,22.5cqw,18rem)] font-black leading-[0.85] tracking-[-0.015em] opacity-10 dark:opacity-20 uppercase font-display">
          {heroTitle}
        </span>
      </div>
      ...
      {books.length === 0 && (
        <div className="absolute inset-0 z-10 flex items-center justify-center p-8 text-center text-sm text-current opacity-60">
          Add at least one journal book to display the showcase.
        </div>
      )}
      ...
      {showNav && (
        ...
        >
          <div className="text-[clamp(18px,2.2cqw,26px)] font-bold tracking-tight text-current uppercase font-display flex items-center space-x-2">
      ...
      {canCarousel && (
        <>
          <button
            ...
            className={`absolute left-3 top-1/2 z-30 -translate-y-1/2 inline-flex h-11 w-11 items-center justify-center rounded-full bg-white/90 dark:bg-zinc-900/90 text-zinc-900 dark:text-white shadow-xl backdrop-blur-md transition-all duration-300 hover:scale-110 active:scale-95 cursor-pointer border border-white/20 @min-[768px]:left-6 @min-[768px]:h-12 @min-[768px]:w-12 ${uiMode === 'hero' ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'
              }`}
          >
            <ChevronLeft />
          </button>
          <button
            ...
            onClick={() => shiftCarouselRef.current(1)}
            className={`absolute right-3 top-1/2 z-30 -translate-y-1/2 inline-flex h-11 w-11 items-center justify-center rounded-full bg-white/90 dark:bg-zinc-900/90 text-zinc-900 dark:text-white shadow-xl backdrop-blur-md transition-all duration-300 hover:scale-110 active:scale-95 cursor-pointer border border-white/20 @min-[768px]:right-6 @min-[768px]:h-12 @min-[768px]:w-12 ${uiMode === 'hero' ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'
              }`}
          >
            <ChevronRight />
          </button>
      ...
      <button
        ...
        className={`absolute left-1/2 top-[30px] z-40 -translate-x-1/2 inline-flex h-[52px] w-[52px] items-center justify-center rounded-full border-[1.5px] border-[var(--bs-cream)]/40 bg-zinc-900/80 backdrop-blur-xl text-[17px] leading-none text-[var(--bs-cream)] transition-[opacity,border-color] duration-300 delay-150 hover:border-[var(--bs-cream)]/90 @max-[760px]:left-auto @max-[760px]:right-[18px] @max-[760px]:top-[18px] @max-[760px]:translate-x-0 [-webkit-tap-highlight-color:transparent] cursor-pointer shadow-2xl ${uiMode === 'detail' ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'
          }`}
      >
        &#10005;
      </button>
      ...
      {showDetailPanel && (
        ...
        >
          <div className={`text-[11px] font-mono tracking-[0.2em] text-rose-400 font-bold uppercase mb-2 ${dpChild(50)}`}>
            {selectedCfg?.category || 'INEFFABLE JOURNAL'}
          </div>

          <h1 className={`m-0 text-white text-[clamp(36px,4.5cqw,68px)] font-display font-extrabold leading-[1.05] tracking-tight @max-[760px]:text-[clamp(28px,8.5cqw,44px)] drop-shadow-md ${dpChild(50)}`}>
            {selectedCfg?.title}
          </h1>

          <p className={`mt-[18px] max-w-[54ch] text-zinc-300 text-[clamp(14px,1.15cqw,17px)] leading-[1.65] font-light @max-[760px]:mt-3 @max-[760px]:line-clamp-4 ${dpChild(130)}`}>
            {selectedCfg?.desc}
          </p>

          <div className={`mt-[24px] flex items-center gap-[18px] @max-[760px]:mt-[14px] ${dpChild(210)}`}>
            <div className="flex gap-[4px]">
              {[0, 1, 2, 3, 4].map((i) => (
                <svg
                  key={i}
                  viewBox="0 0 24 24"
                  className={`h-[20px] w-[20px] fill-rose-500 ${i < (selectedCfg?.stars ?? 5) ? '' : 'opacity-25'}`}
                >
            ...
            <div className="text-sm font-mono text-zinc-400">BY {selectedCfg?.author?.toUpperCase()}</div>
            <div className="ml-auto text-sm font-mono text-zinc-400">{selectedCfg?.year || selectedCfg?.date}</div>
          </div>

          <div className={`mt-[20px] border-t border-white/10 @max-[760px]:mt-3 ${dpChild(270)}`} />

          <div
            className={`pointer-events-auto mt-6 inline-flex items-center gap-[12px] rounded-full bg-zinc-950/80 backdrop-blur-2xl p-2.5 border border-white/15 shadow-[0_24px_60px_rgba(0,0,0,0.6)] @max-[760px]:mt-[14px] @max-[760px]:flex-wrap @max-[760px]:rounded-[28px] ${dpChild(330)}`}
          >
            <button
              onClick={() => selectedCfg && onOpenReader?.(selectedCfg)}
              className="inline-flex h-[50px] items-center gap-[8px] rounded-full bg-rose-500 hover:bg-rose-600 px-[24px] text-sm font-bold text-white transition-all duration-200 hover:scale-[1.03] active:scale-95 cursor-pointer shadow-lg shadow-rose-500/25"
            >
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="h-4 h-4">
